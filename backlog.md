...
| BL-UAT-13 | Resolve polling place inside the lookup result | P2 | L | P1 Q1.3 "Where do I vote?" | Currently the lookup returns ward + races + council member, then offers a generic external link `Find your polling place at DCBOE ↗`. Clicking sends the user to dcboe.org with the address field empty — they have to re-type. **Fix path**: extend the corsproxy-routed MAR query to also call DCBOE's polling-place endpoint (or a known mapping `ward → polling place` table fallback). Lands as a `Your polling place: <address>` line inside the existing result block. If DCBOE has no CORS-friendly endpoint, fall back to a deep-link with the address pre-filled in the query string (DCBOE site supports `?address=` pattern — verify before commit). |
...
| BL-UAT-15 | Auto-derive candidate counts in race `oneLine` (resilience fix) | P3 | M | P2 Q2.1 "Who's running for X?" | UAT-017 is the symptom (Mayor `oneLine` says "10 declared Democrats" while data has 8). Root cause: counts are hardcoded into `Race.oneLine` strings, so they drift every time the data-refresh skill adds or drops a candidate. Two paths: (a) split the oneLine into `Race.summary` (count-free copy) + a derived count rendered by the UI (`{candidatesForRace(slug).filter(...).length} declared`), or (b) add a unit test that asserts every `oneLine` numeric count matches the live filter. Path (a) is the durable fix; path (b) is a 30-line guard. Audit all 5 races whose oneLine cites a count: `mayor`, `council-chair`, `council-at-large-bonds`, `us-house-delegate`, `council-at-large-special`. |
