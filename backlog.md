...
|---|---|---|---|---|---|
| BL-59 | Agent research run for newly profiled races (council-chair, attorney-general, council-at-large-special, wards 3/5/6, shadow-senator): positions on the 6 tracked issues, campaign websiteUrl, news refresh. All 11 races with candidates got race + candidate pages on 2026-06-11 from existing data only; their comparison matrices are hidden until positions land. Use the per-race agent fan-out pattern and source map saved in memory (position-research-playbook + candidate-source-cache): campaign-site-first, WTOP get-to-know Q&A pattern, agents write JSON to disk, merge via scripts/merge-positions.mjs. Also: profile shadow-representative once a candidate is on file. | M | M | ★★★ | 16 candidate pages currently show summaries + filings but no positions; primary is June 16. |
...
|----|-------|----------|--------|-----------|
| BL-UAT-19 | Reconcile stale references in CLAUDE.md + both skill files with the current code | P3 | S | Documentation drift confirmed this run. (1) `CLAUDE.md` file-map still describes `NavBar.tsx` as `"use client"` with a `<details>` hamburger drawer (UAT-016) — but BL-47 redesigned it to a 2-item server-component nav (Issues/Elections) with CSS hover popouts and no hamburger; the Tech-invariants "client-side JS that hydrates" list still names NavBar's onClick handler, which no longer exists. (2) Both `~/.claude/skills/dc-data-refresh.md` and `dc-uat.md` reference the pre-`dfebf0a` `dc-elections-tracker/` subdirectory paths (real paths are repo-root `src/data/…`), Next.js 14.2.13 (now Next 16 / React 19 per `ae88214`), and launch configs `dc-elections-tracker-dev` / `-static` (actual config in `.claude/launch.json` is `dc-watch-dev`). A future automated run that follows the skills literally could fail to locate files or start the server. Cross-refs BL-UAT-17 (nav-specific BF-05/BF-06 rewrite). Skills live outside the repo, so the human owner must apply those edits; CLAUDE.md is in-repo and safe to fix directly. **UPDATE 2026-05-21 (run 10):** the CLAUDE.md NavBar drift in (1) is now fixed — the file-map entry and the Tech-invariants "client-side JS that hydrates" line both describe the BL-47 server-component nav. Only the external skill-file edits in (2) remain (subdir paths, Next 14.2.13 → 16, `dc-watch-dev` launch config); those stay with the human owner. **NOTE 2026-05-23 (run 11):** an untracked `AGENTS.md` now sits at the repo root (a Codex-oriented mirror of CLAUDE.md). Its `~/.Codex/skills/dc-uat.md` references are the same drift in a different guise; if AGENTS.md is meant to be canonical for Codex, fold its path/version reconciliation into this item. |

## UAT-sourced improvements (added 2026-05-23, run 11 — scheduled morning pass)

Paired with data-refresh run 11 (thin ~2-day window: registration-deadline alert + Axios delegate piece). Verified against the static `out/` export because the live dev server couldn't bind. Zero new product bugs.

| ID | Title | Priority | Effort | Rationale |
|----|-------|----------|--------|-----------|
| BL-UAT-20 | Pin a project-specific port in `.claude/launch.json` so scheduled UAT runs don't collide with other projects' dev servers | P3 | S | This run the `dc-watch-dev` preview server couldn't bind: a stale `next dev` from another local project (DryDock, next-server v15.5.18) was already holding port 3000, and although the preview wrapper reported assigning autoPort 60751, the spawned `next dev` didn't end up listening there — the browser got connection-refused on every navigation. The scheduled `daily-dcelection-refresh` runs unattended, so it can't fall back to a human killing the other process (and shouldn't — that's another project's work). Two cheap options: (a) give the launch config a less-contended fixed port (e.g. `3850`) plus `runtimeArgs: ["run","dev","--","--port","3850"]` so `next dev` actually honors it; or (b) have the UAT routine prefer a `serve out/` static config (already the skill's recommended pre-commit path) which has no compile step and is less port-sensitive. Without this, every scheduled run that overlaps another project's dev server loses its live-browser interactive pass (RCV simulator, address lookup) and can only verify the static artifact. Found UAT run 11 (2026-05-23). **Partial mitigation shipped run 15 (2026-05-26):** option (b) now exists in `.claude/launch.json` as `dc-watch-static` (`npx serve -l 3000 out`, autoPort enabled) and was used cleanly by run 15. Option (a) (a fixed less-contended port for `dc-watch-dev`) and the underlying interactive-component live-browser hole remain open for a future run. |

## UAT-sourced improvements (added 2026-05-24, run 12 — scheduled morning pass)

Paired with data-refresh run 12. Headline finding was a source-integrity near-miss: an abandoned earlier run-12 attempt had left uncommitted candidate-news items with fabricated forum-recap URLs in the working tree (see UAT-021 in `issues.md`). They were caught at Step 0, URL-probed (all 404 / refused), stashed, and the one legitimate development (the 2026-05-21 GWU mayoral forum) was re-added with its verified `gwtoday.gwu.edu` URL.

| ID | Title | Priority | Effort | Rationale |
|----|-------|----------|--------|-----------|
| BL-UAT-21 | Add a URL-liveness guard so fabricated/dead source URLs can't reach a commit | P2 | M | UAT-021 this run exposed a failure mode unique to the automated refresh: a partial/abandoned run can leave fabricated source URLs in `elections.ts` (or any data file) that look plausible but 404. The editorial promise ("every claim links to a primary or authoritative source") is the project's core invariant, so a dead/fabricated URL is a P-class data-integrity defect, not cosmetic. Options, cheapest first: (a) a `scripts/check-links.mjs` that extracts every `url:`/`sourceUrl:`/`href:` from `src/data/*.ts` + the RCV page and HEAD-requests each, failing on non-2xx — run it manually at the end of each refresh and optionally in CI (note: some outlets 403 bots, so allow a domain skiplist + treat 403/405 as "reachable"); (b) make it part of the data-refresh skill's Step 8 checklist so the agent must probe each *newly added* URL before writing it (this run did that by hand). (a) is the durable fix; (b) is the immediate behavioral one. Cross-refs the editorial promise in CLAUDE.md. Found UAT run 12 (2026-05-24). |
| BL-UAT-22 | Enrich the 4 newly-added at-large (Bonds seat) candidates with `news[]` + `positions` | P2 | M | Run 13 added Dwight Davis, Dyana N.M. Forester, Fred Hill, and Greg Jackson to `candidates2026` (council-at-large-bonds) from the East of the River roundup, but seeded them with only name/party/source/website/notes — no `news[]`, `newsThemes`, or comparison `positions`. Because this is a PROFILED race, each now has a live `/elections/council-at-large-bonds/<slug>/` profile page that currently shows an empty Coverage section and "0 of 6" positions, and they're absent from the `/elections/` comparison matrix cells. Next refresh should run the Step 2.5 per-candidate news pass (Tier A) and a positions pass for all four — Forester (AFL-CIO labor lane) and Jackson (gun-violence-prevention/federal background) are the highest-signal. Found run 13 (2026-05-24). |
| BL-UAT-23 | Reconcile the Nate Fleming at-large entry against the authoritative DCBOE candidate PDF | P3 | S | Our `candidates2026` carries Nate Fleming (council-at-large-bonds, "former shadow representative", HillRag 2026-01-15) as `declared`, but the 2026-05-01 East of the River at-large roundup — the most recent full roster we have — lists 9 declared Democrats and does **not** include Fleming. Absence from one roundup is not proof of withdrawal, so this run left him as-is (the conservative call). Next run should diff against the authoritative DCBOE 2026 primary candidate PDF: if Fleming is on it, keep `declared` and add `dcboeUrl`; if he never filed or withdrew, set `filingStatus: "withdrawn"` with a sourced note (which would drop the tracked count from 10 to 9 and require updating the race oneLine). Found run 13 (2026-05-24). |
| BL-UAT-24 | Decide what to do with the orphan `electionStats` array — re-surface or delete | P1 | S | UAT-022 (run 16, 2026-05-27): PR #19's voter-first restructure removed the `electionStats.map(…)` render from `/elections/` but left the 3-entry data array (478,797 reg, May 11 mail, May 22 drop boxes) intact. The data-refresh skill's Step 1a explicitly maintains the registration-statistics figure each month, so the skill has been doing work that ships nothing. Two options: (a) re-surface the 478,797 figure in a voter-relevant place — the home `TodayBanner` already triggers off `importantDates`, so a small "Active registered voters in DC" stat tile on `/elections/` or in the home hero would restore the maintained data with high voter-value; or (b) delete the orphan array and update the dc-data-refresh skill's Step 1a/Step 0 instructions to drop registration-PDF tracking. **Prefer (a)** — 478,797 is the cleanest "this is your electorate" number on the site, and it answers a real voter question ("how many people am I competing with for my candidate's slate?"). P1 because the maintenance work happens on every refresh; the longer we wait, the more wasted skill cycles. Found run 16. |
| BL-UAT-25 | Voter-guide aggregator surface on race pages — link Barred in DC / ACLU / GLAA / LWV style multi-candidate questionnaires without per-candidate `news[]` slots | P2 | M | Run 20 (2026-06-02): the Barred in DC mayoral questionnaire (2026-05-27, 6 of 7 mayoral candidates responding) and at-large questionnaire (2026-05-26, 9 at-large candidates) were noted but not added because both flagship mayoral profiles are at cap=12 and these multi-candidate questionnaires are awkward to encode as per-candidate `news[]` items — they answer "what every candidate thinks about X" rather than "what did people write about candidate Y." The existing BL-45 spec proposes a `VoterGuide` type rendered on race pages; this item is a narrower P2 ask to ship just the `VoterGuide` list (drop the `CandidateForum` half for v1) so multi-candidate questionnaires can be linked from `/elections/[race]/` once and credited to all candidates. Sources to seed: Barred in DC (mayoral + at-large), ACLU-DC, GLAA, DC YIMBYs, MDC DSA — all live and HEAD-200 as of run 20. Found run 20 (2026-06-02). |
