# CLAUDE.md — TicketPriceTracker

> Cross-platform ticket price comparison tool with React Native mobile app.
...
### Architecture
- **Backend:** Python FastAPI + SQLite (`backend/api.py`, `backend/database.py`)
- **Scrapers:** Modular per-platform scrapers in `backend/scrapers/` with shared `BaseScraper` ABC
- **Mobile:** React Native Expo app in `mobile/` with TypeScript
- **Data flow:** Scraper → SQLite → FastAPI → React Native

### Quick Start
```bash
./start.sh           # Full stack (API + Expo)
./start-backend.sh   # API only (port 8000)
./start-mobile.sh    # Expo only
```
