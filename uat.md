...
_Last run: 2026-05-21 morning (UAT run 10 — scheduled `daily-dcelection-refresh` morning pass; paired with data-refresh run 10. Zero new product bugs. Verified the first RCV-era primary poll (City Cast / TrueDot) renders as the top alert, the new RCV recentMove, and the lead news item on both the JLG and McDuffie profiles — both held cap=12 with the oldest item correctly dropped. Static build emits all 71 routes; 119 tests + typecheck clean. Fixed the in-repo CLAUDE.md half of BL-UAT-19. See run-10 log entry below.)_
...
| Homepage — alert ticker | 2026-05-21 | Stable. CSS marquee + prefers-reduced-motion guard. New top alert (City Cast/TrueDot poll) renders; no overflow at 375px. |
